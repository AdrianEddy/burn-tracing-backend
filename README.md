...
## SKILL.md — LLM Agent Reference

[`SKILL.md`](SKILL.md) is a standalone reference designed for LLM agents working on ML model optimization and profiling. Trace files can contain hundreds of thousands of events and are too large for an LLM to read directly. SKILL.md gives the agent everything it needs to write targeted Python scripts that parse only the relevant parts of `trace_data.js`:

- Complete JSON schema for every event field (core, fusion, marker, sync, shapes, tensor data)
- Python loader snippet and 10 ready-to-use analysis patterns (bottleneck detection, fusion ratio, memory timeline, matmul shape analysis, etc.)
- Full Rust API reference (backend setup, tracing lifecycle, markers/spans, tensor capture)
- Timing model explanation (CPU dispatch vs GPU execution, sync semantics)

Point your agent at `SKILL.md` and it can immediately start writing analysis code without needing to read the source.
